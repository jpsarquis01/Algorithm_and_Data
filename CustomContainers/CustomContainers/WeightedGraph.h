...
public:
	...
	struct Node
	{
		...
		Node* parent = nullptr;
		// total weight for tracking when the path finding
		W totalWeight;
	};

	// this "public" has no value other than organization
public:
	WeightedGraph()
	{
	...
	W GetPath(int startNode, int endNode, Vector<const T*>& pathOutput)
	{
		...
		while (!process.empty())
		{
			...
			for (std::size_t i = 0; i < node->edges.Size(); ++i)
			{
				...
				// checks if edge cna be processed (under the max limit, since max + weight is valid)
				// check to see if shorter distance, if so store, ohterwise ignore
